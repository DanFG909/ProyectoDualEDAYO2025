...
class GenerarPDF {
    ...
    private function obtenerDatos() {
        $sql = "SELECT id, Nombre, Informacion, Modalidad, fecha_creacion FROM cursos";
    ...
    public function crearPDF() {
        ...
        if (empty($rows)) {
            ...
            foreach ($rows as $r) {
                $html .= '<tr>
                    ...
                    <td>' . htmlspecialchars($r['Informacion'], ENT_QUOTES, 'UTF-8') . '</td>
                    <td>' . htmlspecialchars($r['Modalidad'], ENT_QUOTES, 'UTF-8') . '</td>
                    <td>' . htmlspecialchars($r['fecha_creacion'], ENT_QUOTES, 'UTF-8') . '</td>
                </tr>';
            }
        }
        ...
        $dompdf->render();
        $dompdf->stream("TablaCursos.pdf", ["Attachment" => false]);
    }
}
$pdf = new GenerarPDF();
$pdf->crearPDF();
